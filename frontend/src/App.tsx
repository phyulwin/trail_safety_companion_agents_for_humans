...
export default function App() {
  ...
  useEffect(() => {
    api<{public_demo: boolean}>('/health').then(data => setPublicDemo(data.public_demo)).catch(() => {});
  ...
  async function startTrail() {
    ...
    if (publicDemo) { await startDemo('normal'); return; }
  ...
  return <div className="app-shell">
    ...
    <div className="main-shell"><header className="topbar"><div className="breadcrumb">Your personal safety companion<span>/</span><strong>{page === 'home' ? 'Overview' : page === 'retrace' ? 'Lost item search' : nav.find(n => n.id === page)?.label || 'Profile'}</strong></div><div className="top-actions"><span className="local-pill"><span className="live-dot"/>{session?.state.agent_status === 'ERROR' ? 'BEDROCK ERROR' : session?.state.agent_status === 'RUNNING' ? 'AGENT WORKING' : session?.state.agent_status === 'COMPLETE' && session.agent_mode === 'bedrock' ? 'BEDROCK VERIFIED' : 'TRAIL DEMO READY'}</span><button className="icon-button" aria-label="Open activity" onClick={() => {setPage('home'); document.getElementById('activity')?.scrollIntoView({behavior: 'smooth'});}}><Bell size={19}/></button><button className="avatar" aria-label={user ? 'Open profile' : 'Sign in'} onClick={() => user ? setPage('profile') : setAuthOpen(true)}>{user?.display_name.slice(0,1) || 'T'}</button></div></header>
      ...
      {session?.state.agent_status === 'ERROR' && <div className="message error" role="alert">Bedrock agent failed ({session.state.agent_error}); any safety policy actions are labelled separately in activity.</div>}
      ...
      {page === 'home' && <>
        <div className="overview-grid"><div className="main-column">
          ...
          <section className="demo-card"><span className="eyebrow"><Play size={13}/>TAKE TRAIL FOR A TEST RUN</span><h3>See care in action.</h3><p>Watch a simulated run move from a quiet check-in to a community helping hand.</p><button className="button outline full" disabled={busy || active} onClick={() => void startDemo('safety')}><Play size={15}/>Run Safety Demo<ArrowRight size={15}/></button><button className="text-button demo-normal" disabled={busy || active} onClick={() => void startDemo('normal')}>Run Normal Demo <ArrowRight size={13}/></button><small>Enables sample contact sharing & community opt-in.<br/>Simulated data · in-app alerts; no GPS permission needed.</small></section>
        </aside></div>
      </>}
