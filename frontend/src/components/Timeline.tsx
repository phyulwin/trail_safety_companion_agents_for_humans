...
export function Timeline({session}: {session: TrailSession | null}) {
  ...
  const rows = session ? [
    ...
    ...session.decisions.filter(decision => !['NO_ACTION', 'ANALYSIS_COMPLETE', 'TOOL_EXECUTED', 'TOOL_REJECTED'].includes(decision.action)).map(decision => ({id: decision.id, created_at: decision.created_at, title: decision.action.split('_').join(' '), description: `${decision.mode}: ${decision.explanation}`, agent: decision.mode !== 'policy'})),
  ...
  return <section className="card timeline-card"><div className="section-label"><span><Radio size={16}/> Trail activity</span><span className="tiny-label">LIVE FEED</span></div>
    ...
    {session && <details className="tool-audit"><summary>View actual Strands tool executions ({session.decisions.filter(d => d.action === 'TOOL_EXECUTED').length})</summary>{session.decisions.filter(d => ['TOOL_EXECUTED', 'TOOL_REJECTED'].includes(d.action)).map(d => <div key={d.id}><strong>{d.metrics.tool}</strong><small> · {d.mode} · {clockTime(d.created_at)}</small><pre>{JSON.stringify(d.metrics.output, null, 2)}</pre></div>)}</details>}
    {session && <div className="timeline-footer"><BrainCircuit size={14}/>{session.agent_mode === 'mock' ? 'Strands Â· deterministic demo model' : 'Strands Â· Amazon Bedrock'}<span>{session.decisions.filter(d => d.action === 'ANALYSIS_COMPLETE').length} analyses</span></div>}
  </section>;
}
